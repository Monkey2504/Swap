...
import { Upload, Camera, Trash2, Calendar, ChevronRight, X, MapPin, Loader2, User, FileText } from 'lucide-react';
...
const ProfilePage: React.FC<{ onNext: () => void; duties: Duty[]; dutiesLoading: boolean }> = ({ onNext, duties, dutiesLoading }) => {
  ...
  const processRosterData = async (base64Data: string, mimeType: string) => {
    if (!user) return;
    setIsUploading(true);
    setUploadProgress("Initialisation IA...");
    try {
      ...
      if (services?.length) {
        for (const s of services) {
          await addDuty({
            ...
            relations: [], destinations: s.destinations || [], compositions: []
          });
        }
        setSuccessMessage(`${services.length} prestations ajoutées.`);
      }
      ...
      {showCamera && (
        <div className="fixed inset-0 bg-slate-900/95 z-[100] flex flex-col items-center justify-center p-6 backdrop-blur-xl">
          ...
          <div className="mt-12 flex items-center gap-8">
            <button onClick={stopCamera} className="w-16 h-16 bg-white/10 text-white rounded-full flex items-center justify-center"><X size={24} /></button>
            <button onClick={() => {
            ...
            }} className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-2xl border-8 border-white/20 active:scale-90 transition-all"></button>
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <div className="w-full lg:w-80 space-y-8">
          <div className="bg-white rounded-[48px] p-10 shadow-2xl border border-white text-center">
            <div className="w-32 h-32 bg-sncb-blue rounded-[40px] flex items-center justify-center text-white text-4xl font-black mx-auto mb-8 shadow-xl">
              {user.firstName[0]}
            </div>
            
            {isEditing ? (
              <div className="space-y-4">
                <input type="text" value={editFirstName} onChange={e => setEditFirstName(e.target.value)} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none font-bold text-sm" placeholder="Prénom" />
                <input type="text" value={editLastName} onChange={e => setEditLastName(e.target.value)} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none font-bold text-sm" placeholder="Nom" />
                <select value={editDepot} onChange={e => setEditDepot(e.target.value)} className="w-full px-5 py-4 rounded-2xl bg-slate-50 border-none font-bold text-sm">
                  {DEPOTS.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
                <button onClick={() => { updateUserProfile({firstName: editFirstName, lastName: editLastName, depot: editDepot, onboardingCompleted: true}); setIsEditing(false); }} className="w-full py-5 bg-sncb-blue text-white rounded-2xl font-black text-sm shadow-xl shadow-sncb-blue/10 mt-4">SAUVEGARDER</button>
              </div>
            ) : (
        ...
        <div className="flex-grow space-y-12">
          ...
          <div className="bg-white rounded-[48px] p-10 shadow-2xl border border-white relative overflow-hidden">
            {isUploading && (
              <div className="absolute inset-0 bg-white/90 z-20 backdrop-blur-md flex flex-col items-center justify-center animate-fadeIn">
                <Loader2 className="w-10 h-10 text-sncb-blue animate-spin mb-4" />
                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-900">{uploadProgress}</p>
              </div>
            )}
            ...
            <div className="flex items-center gap-4 mb-10">
              ...
              <div>
                <h3 className="text-xl font-black tracking-tight italic">Chargement Roster IA</h3>
                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Digitalisation SNCB Premium</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <button onClick={startCamera} className="p-10 rounded-[40px] border-2 border-dashed border-slate-100 hover:border-sncb-blue hover:bg-slate-50 transition-all flex flex-col items-center gap-4 group">
                ...
                <span className="font-black text-[11px] uppercase tracking-widest text-slate-900">Scan Planning Papier</span>
              </button>
              ...
              <button onClick={() => fileInputRef.current?.click()} className="p-10 rounded-[40px] border-2 border-dashed border-slate-100 hover:border-emerald-500 hover:bg-slate-50 transition-all flex flex-col items-center gap-4 group">
                ...
                <span className="font-black text-[11px] uppercase tracking-widest text-slate-900">Importer PDF / Image</span>
              </button>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {duties.map(d => (
                <div key={d.id} className="bg-white p-6 rounded-[32px] shadow-xl border border-white flex items-center justify-between group">
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 bg-slate-50 rounded-[20px] flex flex-col items-center justify-center border border-slate-100">
                  ...
                  <button onClick={() => removeDuty(d.id)} className="p-3 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-xl opacity-0 group-hover:opacity-100 transition-all">
                    <Trash2 size={18} />
                  </button>
                </div>
              ))}
