...
import { Settings, Eye, EyeOff, Lock, Mail, HelpCircle, Loader2 } from 'lucide-react';
...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // La redirection vers Google se fait ici
    } catch (error: any) {
      setAuthError(formatError(error));
      ...
      <div className="w-full max-w-sm px-6 relative z-10 animate-fade-in flex flex-col items-center">
            ...
            {showSettings ? (
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full py-4.5 bg-[#2162c1] hover:bg-sncb-blue text-white rounded-2xl font-bold text-base shadow-lg shadow-sncb-blue/20 active:scale-[0.98] transition-all flex items-center justify-center disabled:opacity-50"
                >
        ...
        <div className="w-full space-y-3 mt-4">
          <button 
            ...
            disabled={loading || googleLoading}
            className="w-full py-4 bg-white rounded-2xl flex items-center px-6 gap-4 shadow-xl active:scale-95 transition-all disabled:opacity-50"
          >
             {googleLoading ? (
               <Loader2 className="w-5 h-5 animate-spin text-sncb-blue" />
             ) : (
