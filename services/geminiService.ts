...
// NOTE: Dans un environnement Next.js / Vercel, la clé API doit être
// préfixée par NEXT_PUBLIC_ pour être accessible dans le navigateur (Front-End).
// Si vous déplacez cette fonction vers une Route API, utilisez process.env.API_KEY
const GEMINI_API_KEY = process.env.NEXT_PUBLIC_GEMINI_API_KEY || process.env.NEXT_PUBLIC_API_KEY;

if (!GEMINI_API_KEY) {
    // Ceci s'affichera dans la console du navigateur si la clé est manquante
    console.error("ERREUR CRITIQUE: NEXT_PUBLIC_GEMINI_API_KEY n'est pas définie. L'IA ne fonctionnera pas.");
}
...
export async function parseRosterDocument(base64Data: string, mimeType: string): Promise<Duty[]> {
    if (!GEMINI_API_KEY) {
        throw new Error("Clé API Gemini manquante. Vérifiez la variable NEXT_PUBLIC_GEMINI_API_KEY dans votre fichier .env.");
    }

    // Initialisation à l'intérieur de la fonction pour garantir que l'environnement est prêt
